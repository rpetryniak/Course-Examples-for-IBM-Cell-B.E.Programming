...
//obsługa operatora '=' jest identyczna jak konstruktor kopiujący
BMP & BMP::operator= ( BMP& Input)
{ 
 // first, make the image empty.
...
*/
 ...
 Pixels = new RGBApixel* [ Width ]; 
 //RGBApixel *data = new RGBApixel [ Width * Height ];  //pierwsza wersja, ciągły obszar pamięci, 
 RGBApixel *data;
 int rc = posix_memalign ((void**)(&data), 128, Width * Height * sizeof(RGBApixel)); //pierwsza wersja, ciągły wyrównany obszar pamięci
 
 for(i=0; i<Width; i++)
 { 
 	Pixels[i] = (RGBApixel *)&data[Height*i];
 }
