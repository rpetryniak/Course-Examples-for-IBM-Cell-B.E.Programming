 ...
 */
unsigned char local_buffer[CHUNK_SIZE] __attribute__ ((aligned (128)));
         ...
         unsigned long long envp __attribute__ ((unused)))
{
  ...
  {
    /* set the in_addr */
    in_addr = control_block.in_addr + (i* CHUNK_SIZE * sizeof(unsigned char));

    /* issue a DMA get command to fetch the chunk of data from system memory */
    mfc_get (local_buffer, in_addr, CHUNK_SIZE * sizeof(unsigned char), tag, 0, 0);
     ...
     * local store*/
    negatyw (local_buffer, CHUNK_SIZE);

   /* issue the DMA put command to transfer result from local memory to 
    * system memory */
    mfc_put (local_buffer, in_addr, CHUNK_SIZE * sizeof(unsigned char), tag, 0, 0);
